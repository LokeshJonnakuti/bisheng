...
def query_file_from_dataset(dataset_id):
    ...
    response = requests.post(url, headers=headers, data=json.dumps(data), timeout=60).json()['data']
    print(response)
...
def create_by_file(file_path, dataset_id):
    ...
    response = requests.post(url, headers=headers, data={'data': json.dumps(data)}, files=files, timeout=60).json()
    print(response)
    return response
